...
async function fn() {
  const result = await p // 接收 Promise 返回成功的值
  console.log(result)
}
...
async function fn() {
  try {
    const result = await sendAJAX(ANIMAL_URL) // 接收 AJAX
    console.log(result)
  }
  catch (err) {
    console.warn(err)
...
async function fn() {
  try {
    const result = await sendAJAX(ANIMAL_URL) // 接收領養動物 AJAX
    const weather = await sendAJAX(WEATHER_URL) // 接收天氣預報 AJAX
    console.log(result)
  }
  catch (err) {
    console.warn(err)
